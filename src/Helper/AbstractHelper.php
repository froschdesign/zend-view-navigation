...
namespace Zend\View\Navigation\Helper;
    ...
    HelperInterface
{
    ...
    protected function parseContainer(&$container = null)
    {
        ...
        if (is_string($container)) {
            $containerManager = $this->getNavigationManager();
            if (! $containerManager) {
                throw new Exception\InvalidArgumentException(sprintf(
                    'Attempted to set container with alias "%s" but no ContainerManager was set',
                    $container
                ));
            }
            ...
            if (in_array($container, ['default', 'navigation'], true)) {
                // Uses class name
                if ($containerManager->has(Navigation\Navigation::class)) {
                    $container = $containerManager->get(Navigation\Navigation::class);
                    return;
                }

                // Uses old service name
                if ($containerManager->has('navigation')) {
                    $container = $containerManager->get('navigation');
                    return;
                }
            }
             ...
             */
            $container = $containerManager->get($container);
            return;
        }
     ...
     *
     * @param ContainerInterface $navigationManager
     */
    public function setNavigationManager(ContainerInterface $navigationManager)
    {
